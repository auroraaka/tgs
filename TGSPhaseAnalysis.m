...
function [frequency_final,frequency_error,SAW_speed,thermal_diffusivity,thermal_diffusivity_err,acoustic_damping_constant, acoustic_damping_error, A, A_err, displacement_reflectance_ratio, displacement_reflectance_ratio_err, B, B_err, acoustic_phase, acoustic_phase_err, C, C_err, start_time2, x_raw, y_raw, x_fit, y_fit] = TGSPhaseAnalysis(pos_file,neg_file,grating,start_point,two_SAW_frequencies,baselineBool,POSbaselineStr,NEGbaselineStr, delimiter, verbose, header_length, plot_things, mono_heterodyne)
...
printFitsToFiles = 1; %outputs both thermal and full fit data to output file
...
if start_point==0
    ...
    if amp_grat==0
        ...
        if ~find_max
            ...
            [fft] = TGS_phase_fft(SAW_only,psd_out,truncate_fraction, plot_things);
            ...
            if tau_final_fit
                ...
                % LB15=[0 low_bound(1) low_bound(2) 0 -2*pi low_t -5e-3 peak_freq*0.5];
                % UB15=[1 up_bound(1) up_bound(2) 10 2*pi up_t 5e-3 peak_freq*1.5];
                % ST15=[f2.A f2.k f2.beta f2.B f2.p f2.t f2.D peak_freq];
                % OPS15=fitoptions('Method','NonLinearLeastSquares','Lower',LB15,'Upper',UB15,'Start',ST15);
                % TYPE15=fittype('A.*(erfc(q*sqrt(k*(x+start_time)))-beta*exp(-q^2*k*(x+start_time))./sqrt((x+start_time)))+B.*sin(2*pi*(F)*(x+start_time)+p)*exp(-(x+start_time)/t)+D','options',OPS15,'problem',{'q','start_time'},'coefficients',{'A','k','beta','B','p','t','D','F'});
                % [f15,gof]=fit(total_signal(start_index2:end,1),total_signal(start_index2:end,2),TYPE15,'problem',{q,start_time2});
                
                % ST2=[f15.A f15.k f15.beta f15.B f15.p f15.t f15.D f15.F];
                % LB2=[0 low_bound(1) low_bound(2) 0 -2*pi low_t -5e-3 0.5*f15.F];
                % UB2=[1 up_bound(1) up_bound(2) 10 2*pi up_t 5e-3 1.5*f15.F];
                % OPS2=fitoptions('Method','NonLinearLeastSquares','Lower',LB2,'Upper',UB2,'Start',ST2);
                % TYPE2=fittype('A.*(erfc(q*sqrt(k*(x+start_time)))-beta*exp(-q^2*k*(x+start_time))./sqrt((x+start_time)))+B.*sin(2*pi*(F)*(x+start_time)+p)*exp(-(x+start_time)/t)+D','options',OPS2,'problem',{'q','start_time'},'coefficients',{'A','k','beta','B','p','t','D','F'});
                % [f2,gof]=fit(total_signal(start_index2:end,1),total_signal(start_index2:end,2),TYPE2,'problem',{q,start_time2});
                % frequency_final = f2.F;

                % if plot_things
                %     figure()
                %     set(gcf,'Position',[0 0 960 540])
                %     plot(total_signal(start_index2:end,1),total_signal(start_index2:end,2))
                %     hold on
                %     title('Jonas Test')
                %     saveas(gcf,strcat(pos_file,"_TGS_Debugging.png"))
                % end
                x_fit = total_signal(start_index2:end,1);
                y_fit = total_signal(start_index2:end,2);
                x_raw = neg(:,1)-time_naught;
                y_raw = pos(:,2)-neg(:,2)-pre_signal_average;


                bounds = confint(f2,0.95);
                % if f2.B < 0.01
                %     disp(strcat('BAD B:  ', num2str(f2.B), ' Conf: ', num2str(bounds(1, 4)),' Tau: ', num2str(f2.t),' Conf: ', num2str(bounds(1, 6)), ' Diff: ', num2str(thermal_diffusivity), ' Beta: ', num2str(beta)));
                % else
                %     disp(strcat('GOOD B: ', num2str(f2.B), ' Conf: ', num2str(bounds(1, 4)),' Tau: ', num2str(f2.t),' Conf: ', num2str(bounds(1, 6)), ' Diff: ', num2str(thermal_diffusivity), ' Beta: ', num2str(beta)));
                % end
            ...
            % csvwrite('test.csv', f2(total_signal(start_index2:end,1)));
            % csvwrite('test2.csv', total_signal(start_index2:end,1));
