...
def find_start_time(signal: np.ndarray, grating_spacing: float, time_step: float, null_point: int = 2) -> Tuple[int, float]:
    ...
    start_time = signal[start_idx, 0]
    return start_idx, start_time


def process_signal(pos_file: str, neg_file: str, grating_spacing: float, heterodyne: str = 'di-homodyne', null_point: int = 2, plot: bool = False) -> Tuple[np.ndarray, float, int, float]:
    """
